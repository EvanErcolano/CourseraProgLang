...
fun number_before_reaching_sum (sum : int, ints : int list) = 
    let 
        fun helper (sum: int, ints : int list, index) =
            let 
                val curr = sum - (hd ints)
            in
                if curr > 0
                then if curr - (hd (tl ints)) > 0
                     then helper(curr, tl ints, index + 1)
                     else index
                else index - 1
            end 
    in
...
fun what_month (doy : int) = 
    ...
    in
       1 + number_before_reaching_sum(doy, days_in_month_list)
    end
...
fun oldest (dates : (int * int * int) list) =
    ...
    then NONE
            ...
            fun oldest_helper_nonempty (dates : (int * int * int) list, curr_oldest : (int * int * int)) =
                if null dates
                then curr_oldest
                else if is_older(hd dates, curr_oldest)
                then oldest_helper_nonempty(tl dates, hd dates)
                else oldest_helper_nonempty(tl dates, curr_oldest)
    in
        SOME (oldest_helper_nonempty(dates, (1000000, 1000000, 1000000)))
    end
